...
Goal: To determine the most optimal post-graduation city to live in by predicting rental prices using data mining algorithms. The project evaluates the difference between income and next year’s rent as key decision factors.

Description: The project applies various data mining models, including generalized linear models, elastic net regression, support vector machines (SVMs), decision trees, gradient boosting, neural networks, and KNN models. Using a dataset on rental and housing data by zip codes, the project predicts future rental prices. The data was preprocessed with log normalization and removal of missing values. Cross-validation was employed to compare model performance, with RMSE as the key evaluation metric.

Skills:

- Data Preprocessing: Log normalization of skewed variables and handling missing values.
- Statistical Modeling: Applying diverse machine learning models like decision trees, random forests, and SVMs.
- Evaluation Metrics: Using RMSE and MAE to assess model accuracy.
- Visualization: Generating importance plots to identify influential predictors.
- Technology: Tools like R for statistical modeling, visualization, and analysis.
  
Conclusion:

- Model Performance: The random forest model was identified as the best-performing approach, with lower RMSE and MAE compared to decision trees.
- Key Predictors: Variables like rent_max and rent_mean were most influential in predicting rental prices.

Results:
The top zip codes with favorable income-to-rent differences were identified as 30327, 28207, 38139, 40059, and 37027.
This project highlights the value of data mining algorithms in analyzing real-world datasets, effectively predicting rental prices, and providing actionable insights for decision-making regarding rental properties.
